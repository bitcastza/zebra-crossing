...
function drawTable (weeksBooked, paginationOffset, dayRange, timeslots, schedule) {
  ...
  const visibleSchedule = schedule.slice(paginationOffset, paginationOffset + dayRange)

  for (let i = 0; i < visibleSchedule.length; i++) {
    for (let j = 0; j < visibleSchedule[i].length; j++) {
      if (visibleSchedule[i][j]) {
...
function processScheduleTable (start, end, timeslots, schedule) {
  ...
  document.getElementById('next-button').onClick(function () {
    ...
    if (paginationOffset > weeksBooked[weeksBooked.length - 1].length - dayRange) {
